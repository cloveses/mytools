...
def main():
    ...
    input('手工登录完成？')
    datas = get_file_datas('台帐0921.xlsx')
    for data in datas:
        deal_tele(data)
        if not all(data[20:29]):
            continue
        print(data[4])
        ...
        # br.switch_to_frame('right')
        # br.switch_to_frame('LowerHalf')                 ctl00_ContentPlaceHolder_GridView_StudentList_ctl02_linkButton_Result
        ...
        WebDriverWait(br, 20).until(EC.presence_of_element_located((By.ID, 'ctl00_ContentPlaceHolder_DropDownList_HCJG')))
        ...
        if not data[20]:
            all_guardians[0] = data[23] or data[26]

        if not data[21]:
            all_guardians[1] = data[24] or data[27]
        ...
        if not data[23]:
            all_guardians[3] = data[20] or data[26]

        if not data[24]:
            all_guardians[4] = data[21] or data[27]
        ...
        if not data[26]:
            all_guardians[3] = data[20] or data[23]

        if not data[27]:
            all_guardians[4] = data[21] or data[24]

        if not data[28]:
            all_guardians[5] = data[22] or data[25]
